...
def config(tmp_path, mode="tiny_overfit"):
    return {
        "name": "synthetic-" + mode,
        "seed": 7,
        "mode": mode,
        "protocol": "tiny_overfit" if mode == "tiny_overfit" else "smoke",
        "device": "cpu",
        ...
        "training": {
            ...
            "epochs": 30,
            "gradient_clip": 1.0,
            "max_train_batches": 2 if mode == "smoke" else None,
            "max_validation_batches": 1 if mode == "smoke" else None,
            "max_test_batches": None,
            "target_accuracy": 0.95 if mode == "tiny_overfit" else None,
...
def test_synthetic_tiny_overfit_and_artifacts(tmp_path):
    cfg = config(tmp_path)
    ...
    bundle = DatasetBundle(
        ...
        None,
        {"train": list(range(32))},
        {"protocol": "tiny_overfit", "official_test_accessed": False, "scientific_result": False},
    )
...
def test_smoke_limits_and_test_waits_for_selection(tmp_path):
    cfg = config(tmp_path, "smoke")
    data = SyntheticEvents(64)
    run_path = Path(cfg["output_root"])
    placeholder = DatasetBundle(
        data, data, None, {}, {"protocol": "smoke", "official_test_accessed": False, "scientific_result": False}
    )
    run = initialize_run(cfg, {}, "smoke command")
    ...
    full = config(run_path / "full", "smoke")
    full["mode"] = "full"
    full["protocol"] = "thesis_clean"
    full["training"]["epochs"] = 1
    full["training"]["max_train_batches"] = 1
    full["training"]["max_validation_batches"] = 1
    run2 = initialize_run(full, {}, "full command")
    guarded = SyntheticEvents(4, run2 / "checkpoints/best_validation.pt")
    bundle = DatasetBundle(
        data, data, guarded, {}, {"protocol": "thesis_clean", "official_test_accessed": True, "scientific_result": True}
    )
    train_centralized(LinearEventModel(), bundle, full, run2)
