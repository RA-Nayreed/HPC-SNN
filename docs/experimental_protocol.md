...
Each topology requires its own compatible instrumentation calibration. The calibration allocation and scientific allocation each require exact internal UUID coverage, but separate allocations may use different physical GH200 devices. Compatibility is based on topology, measurement policy, thresholds, the 100 ms interval, and execution commit. Warm-ups are excluded; at least ten alternating measured/unmeasured pairs must have identical updates, median overhead at most 2%, at least 90% accepted interval coverage, no sampling errors, and zero official-test accesses.
