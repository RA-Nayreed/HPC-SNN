...
manifest="${repo_root}/experiments/scheduling_evaluation/manifest.yaml"
logs="${work_dir}/slurm-logs/scheduling_evaluation"
mkdir -p "${logs}" "${work_dir}/runs/scheduling_evaluation" \
    "${work_dir}/generated-evidence/scheduling_evaluation" "${work_dir}/telemetry/scheduling_evaluation"
...
[[ "${scientific_count}" == 18 && "${allocation_count}" == 6 ]] || {
    echo "scheduling matrix must contain 18 executions in six allocations" >&2; exit 2;
}
...
echo "scientific_execution_count=18"
echo "gpus_per_allocation=4"
echo "nodes_per_allocation=1"
...
job_id="$(sbatch --parsable --account="${CSC_PROJECT}" --chdir="${repo_root}" \
    ...
    "${script_dir}/scheduling_evaluation.sbatch")"
job_id="${job_id%%;*}"
[[ "${job_id}" =~ ^[0-9]+$ ]] || { echo "Slurm returned an invalid job identifier" >&2; exit 2; }
echo "scheduling_evaluation_job_id=${job_id}"
