...
Weeks 1–6 have completed execution and evidence collection. Week 6 evaluated scheduling and hierarchical aggregation; both candidates produced valid negative adoption decisions. The established flat ordered aggregation path remains the default, and Week 7 is the next phase.
...
## Week 6 scheduling and hierarchical aggregation

Week 6 completed 18 scheduling executions and 12 two-node hierarchical-reduction executions for SHD and SSC at seeds 37, 47, and 57. Both committed collections are valid, complete, and finding-free. The frozen event-structure cost model was evaluated offline and then online as a scheduler; its SHD improvement did not generalize to SSC, so the final decision is `event_structure_scheduler_not_adopted`. See the [scheduling record](thesis_records/scheduling_evaluation.md) and [committed scheduling summary](results/scheduling_evaluation/scheduling_evaluation_summary.md).

Node-hierarchical grouping reduced logical inter-node bytes, but its small SHD runtime improvement did not extend to SSC and the grouping changed floating-point results. The final decision is `node_hierarchical_reduction_not_retained`; the established `flat_ordered` aggregation path remains the default. See the [hierarchical-reduction record](thesis_records/hierarchical_reduction.md) and [committed hierarchical summary](results/hierarchical_reduction_evaluation/hierarchical_reduction_evaluation_summary.md). These are valid negative scientific results from successful executions, and Week 7 is the next phase.

## Week 7 scaling, non-IID, and measured energy

The locally verified Week 7 implementation declares two separate prospective collections: 24 system-scaling executions and 24 non-IID-severity executions, all at seeds 37, 47, and 57. Scaling compares one GPU, two GPUs, four GPUs on one node, and four GPUs across two nodes while holding label-Dirichlet alpha 0.5 fixed. Non-IID evaluation compares deterministic IID with label-Dirichlet alpha 1.0, 0.5, and 0.1 on one node with four physical GPUs.

Every row keeps `example_count_longest_processing_time`, `flat_ordered`, one process per physical GPU, and CUDA MPS disabled. The implementation records per-device NVML telemetry and integrated energy, timing and load, allocation accounting, official-test metrics, and separate numerical-identity dimensions. Frozen Week 5 models are transfer diagnostics only and never control assignment.

Scientific execution has not occurred, so no Week 7 result or scaling, energy, accuracy, or adoption conclusion exists. Week 1–6 evidence and every existing file under `results/` remain unchanged; those artifacts are not Week 7 evidence. The [prospective protocol](docs/experimental_protocol.md#week-7-scaling-non-iid-and-measured-energy-protocol), [reproduction commands](docs/reproducibility.md#week-7-prospective-execution), [Roihu guide](environment/roihu/README.md#week-7-scaling-and-non-iid-allocations), [scaling record](thesis_records/system_scaling_energy.md), and [non-IID record](thesis_records/non_iid_energy.md) define the unexecuted work.
